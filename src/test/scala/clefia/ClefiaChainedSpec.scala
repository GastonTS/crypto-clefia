...
class ClefiaChainedSpec extends FreeSpec with Matchers with BeforeAndAfter {
  "Clefia with chained configuration" - {
    ...
    "should solve 128-bit key with more than one block" - {
      ...
      val cipherText = List((-567545091, -1686852915, -248937131, 1167365373), (-2039550581, 424567739, 78917109, -1097072512))
    ...
    "should solve 192-bit key with more than one block" - {
      ...
      val plainText = List((0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f), (0xa1397814, 0x289de80c, 0x10da46d1, 0xfa48b38a))
      val cipherText = List((-498585756, -1627222588, -2132958844, -35552851), (-1129809288, 37860135, -127747911, -2011905404))
    ...
    "should solve 256-bit key with more than one block" - {
      ...
      val plainText = List((0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f), (0xa1397814, 0x289de80c, 0x10da46d1, 0xfa48b38a))
      val cipherText = List((-1590069228, 681437196, 282740433, -95898742), (1489792228, 1672367380, -938511681, -1107482608))
    ...
    "should handle 128-bit string key with more than one block" - {
      ...
      val plainText = List((0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f), (0xa1397814, 0x289de80c, 0x10da46d1, 0xfa48b38a))
      val cipherText = List((-608618860, -1029326990, 1355833740, -554241697), (-1447442348, 224699604, -908284807, 804202349))
    ...
    "should handle 192-bit string key with more than one block" - {
      ...
      val plainText = List((0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f), (0xa1397814, 0x289de80c, 0x10da46d1, 0xfa48b38a))
      val cipherText = List((-984243964, -45562810, -659253345, 1611139987), (-1924976483, 1608413481, -178376288, -795460557))
    ...
    "should handle 256-bit string key with more than one block" - {
      ...
      val plainText = List((0x00010203, 0x04050607, 0x08090a0b, 0x0c0d0e0f), (0xa1397814, 0x289de80c, 0x10da46d1, 0xfa48b38a))
      val cipherText = List((630845392, 497378928, -894724412, -1220841694), (-1453606143, -115049568, 375888018, -1994039296))
