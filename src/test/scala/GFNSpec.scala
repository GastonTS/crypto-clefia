...
class GFNSpec extends FreeSpec with Matchers {
  ...
  "A GFN" - {
    ...
    "should multiply square matrix" - {
      "against vectors" in {
        GFN.squareMatrixXVector(GFN.M0, Array(0xa1, 0x47, 0x34, 0x5a)) should be(Array(62, 213, 177, 210))
      }
    }
    ...
    "should apply gfn correctly" - {
      "using gfn4 with 6 rounds" in {
        val input = (0x094082bcL, 0x6561a1beL, 0x3ca9e96eL, 0x5088488bL)
        ...
        GFN.gfn4H(input, keys, 6) should be ((1171750116L, 3609734244L, 3688607617L, 3692111635L))
        GFN.gfn4(input, keys, 6) should be ((1171750116L, 3609734244L, 3688607617L, 3692111635L))
      }

      "using gfn8 with 6 rounds" in {
        val input = (0x417112deL, 0x2d5090f6L, 0xcca9096fL, 0xa088487bL, 0x8a4584b7L, 0xe664a43dL, 0xa933c25bL, 0xc512d21eL)
        ...
        GFN.gfn8H(input, keys, 6) should be ((4006647101L, 3894415849L, 2675728801L, 1500553729L, 516395372L, 1502064311L, 1307738312L, 3183946091L))
        GFN.gfn8(input, keys, 6) should be ((4006647101L, 3894415849L, 2675728801L, 1500553729L, 516395372L, 1502064311L, 1307738312L, 3183946091L))
      }

      "using gfn4Inverse with 6 rounds" in {
        val input = (0xee0e4c21L, 0x822fef59L, 0x4f0e0e20L, 0x232feff8L)
        ...
        GFN.gfn4Inverse(input, keys, 6) should be ((1585078746L, 1407736314L, 3451340836L, 3397394014L))
      }
